...
CHECKPOINT_FILE = "text_adaboost.svd"
...
if CROSS_VALIDATION:
    ...
    plot_cross_validation(scores, "AdaBoost Score")
else:

    if exists_checkpoint(CHECKPOINT_FILE):
        adaboost_classifier = load_object(CHECKPOINT_FILE)
    else:
        param_grid = [{"learning_rate": [0.5, 1.0, 1.5], 'algorithm': ["SAMME", "SAMME.R"]}]
        ...
        """
            grid_search_cv.best_params_ -> {'algorithm': 'SAMME', 'learning_rate': 1.0}
        """
