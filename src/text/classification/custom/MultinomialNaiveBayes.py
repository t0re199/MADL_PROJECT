import numpy as np
from matplotlib import pyplot as plt
from sklearn.metrics import plot_confusion_matrix
from sklearn.model_selection import train_test_split, GridSearchCV, cross_val_score
from sklearn.naive_bayes import MultinomialNB

from Constants import RANDOM_STATE, COLOR_MAP, FOLDS
from plot.Plots import plot_cross_validation
from spio.Models import load_object, exists_checkpoint, save_object
from spio.TextLoading import load_custom_dataset
from text.TextPreProcessing import preprocess_dataset, vectorize_dataset

CHECKPOINT_FILE = "text_mnb.svd"
CROSS_VALIDATION = False

dataframe = load_custom_dataset()

dataset = preprocess_dataset(dataframe.Text.values)
labels = dataframe.Score.values

dataset = vectorize_dataset(dataset).toarray()

X_train, X_test, y_train, y_test = train_test_split(dataset,
                                                    labels,
                                                    test_size=0.30,
                                                    random_state=RANDOM_STATE)

if CROSS_VALIDATION:
    mnb_classifier = MultinomialNB(alpha=1e-3)

    scores = cross_val_score(mnb_classifier,
                             dataset,
                             labels,
                             scoring="accuracy",
                             cv=FOLDS,
                             n_jobs=-0x1)

    plot_cross_validation(scores, "Multinomial Naive Bayes Score")
else:
    if exists_checkpoint(CHECKPOINT_FILE):
        mnb_classifier = load_object(CHECKPOINT_FILE)
    else:

        param_grid = [{"alpha": [1e-2, 1e-3, 1e-4]}]

        mnb_classifier = MultinomialNB(alpha=1e-3)

        grid_search_cv = GridSearchCV(mnb_classifier,
                                      param_grid,
                                      n_jobs=-0x1,
                                      verbose=0x1)
        """
             grid_search_cv.best_params_ -> {'alpha': 0.001}
        """
        #grid_search_cv.fit(X_train, y_train)
        mnb_classifier.fit(X_train, y_train)

        #mnb_classifier = grid_search_cv.best_estimator_
        save_object(mnb_classifier, CHECKPOINT_FILE)
        y_pred = mnb_classifier.predict(X_test)

    disp = plot_confusion_matrix(mnb_classifier, X_test, y_test,
                                 display_labels=np.unique(y_test),
                                 cmap=COLOR_MAP,
                                 normalize="true")

    disp.ax_.set_title("Multinomial Naive Bayes Confusion Matrix")
    plt.show()
